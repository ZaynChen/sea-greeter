...
void
promise_executor(JSCValue *resolve, JSCValue *reject, gpointer user_data)
{
  (void) reject;
  Comm *instance = user_data;
...
GreeterComm_constructor(JSCContext *context)
{
  ...
  instance->_ready_promise = jsc_value_new_promise(context, promise_executor, instance);
 ...
 */
void
GreeterComm_initialize(WebKitWebPage *web_page, JSCContext *js_context)
{

  WebPage = web_page;
  global_context = js_context;
